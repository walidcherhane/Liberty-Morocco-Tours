  ...
  return (
    <>
      ...
      <div className="relative bg-sky-800/30 font-poppins h-[360px]">
        <img
          src={
            tours[Math.floor(Math.random() * tours.length - 1)]?.node.image
              ?.publicUrl
          }
        ...
        <div className="container mx-auto p-8 h-full flex items-center justify-center ">
          <div className="text-4xl lg:text-5xl uppercase font-bold text-center text-white">
            {/* @ts-ignore */}
            ALL <span className="text-blue-500">{pageContext.city}</span>
            {` `} TOURS
          </div>
        </div>
      </div>
...
export const query = graphql`
  query ToursByCity($city: [String]!) {
    allContentfulTour(filter: { cities: { in: $city } }) {
      edges {
        node {
          categories
          cities
